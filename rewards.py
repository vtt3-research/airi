...
def get_sc_reward_weighted_sent_reward(model, feats, sents, attribute, gen_result,
                  meteor_weight=1.0, cider_weight=0.0, bleu_weight=0.0):
    model.eval()
    with torch.no_grad():
        greedy_result, _ = model.sample(Variable(feats), Variable(attribute), greedy=True)
    ...
    sent_lvl_reward = 0.5 + meteor_scores[:num_sample] * 10.0
...
def get_baseline_reward(sents, gen_result, baselines,
                        meteor_weight=1.0, cider_weight=0.0, bleu_weight=0.0):
    ...
    sent_lvl_reward = meteor_scores

    scores = meteor_weight * meteor_scores \
             + cider_weight * cider_scores \
             + bleu_weight * bleu_scores
    base_scores = scores - baselines
    baseline_rewards = np.repeat(base_scores[:, np.newaxis], gen_result.shape[1], 1)

    return sent_lvl_reward, baseline_rewards, scores
